...
namespace LiveCaptionsTranslator.models
{
    public class Setting : INotifyPropertyChanged
    {
        ...
        public string FunAsrServerUrl
        {
            get => funAsrServerUrl;
            set
            {
                funAsrServerUrl = value;
                OnPropertyChanged("FunAsrServerUrl");
            }
        }
        ...
        public Setting()
        {
            ...
            funAsrServerUrl = "http://127.0.0.1:8177";
            sourceLanguage = "ja";
        ...
        {
            string jsonPath = AppDataPaths.ConfigurationPath;
            try
            {
        ...
        public static Setting Load(string jsonPath)
        {
            ...
            // Keep only Ollama when loading settings from older releases.
            var ollamaConfigs = setting.Configs.TryGetValue("Ollama", out var existingConfigs)
                ? existingConfigs.OfType<OllamaConfig>().ToList()
                : [];
            if (ollamaConfigs.Count == 0)
                ollamaConfigs.Add(new OllamaConfig());

            setting.Configs = new Dictionary<string, List<TranslateAPIConfig>>
            {
                { "Ollama", ollamaConfigs.Cast<TranslateAPIConfig>().ToList() }
            };
            setting.ConfigIndices = new Dictionary<string, int>
            {
                { "Ollama", Math.Clamp(setting.ConfigIndices.TryGetValue("Ollama", out int index) ? index : 0, 0, ollamaConfigs.Count - 1) }
            };
        ...
        public void Save()
        {
            Save(AppDataPaths.ConfigurationPath);
        }
        ...
        {
            return File.Exists(AppDataPaths.ConfigurationPath);
        }
    }
}
